...
function GetMailPage() {
  const { authFetch, displayAlert, showAlert } = useGlobalContext();
  ...
  const handleSubmit = () => {
    setIsLoading(true);
    authFetch
      .post("/employee/updatePassword/getEmail", updatePasswordRequest)
      .then((res) => {
        setIsLoading(false);
  ...
  return (
    <div className="mt-32 text-white bg-slate-800 w-[30%] m-auto flex justify-center flex-col items-center rounded-lg">
      ...
      <p className="font-light ">
        Enter your registed email and get reset password link.
      </p>
